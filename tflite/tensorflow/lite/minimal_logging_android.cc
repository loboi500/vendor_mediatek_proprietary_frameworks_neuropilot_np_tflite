...
void MinimalLogger::LogFormatted(LogSeverity severity, const char* format,
                                 va_list args) {
  LogFormatted(severity, "tflite", format, args);
}
void MinimalLogger::LogFormatted(LogSeverity severity, const char* tag, const char* format,
                                 va_list args) {
  // First log to Android's explicit log(cat) API.
  va_list args_copy;
  va_copy(args_copy, args);
  __android_log_vprint(GetPlatformSeverity(severity), tag, format,
                       args_copy);
  va_end(args_copy);

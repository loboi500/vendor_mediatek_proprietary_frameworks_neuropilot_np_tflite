...
/// M: NeuroPilot @{
void DumpOpOutput(tflite::BuiltinOperator builtin_code,
                    std::vector<TfLiteTensor>& tensor_list) {
  bool enable = tflite::mtk::IsDumpOpSupported();
  if (!enable) {
    return;
  }
  ...
  kString += std::string("\n-------------------- Execution ----------------\n")
          + std::string("Device Name      : TFLite CPU")
          + std::string("\nOperations       : ") + opName + "\n";
  ...
  while (it != tensor_list.end()) {
    ...
    if (it->type == kTfLiteUInt8) {
      ...
      float* ptr = it->data.f;
      for (size_t i = 0; i < it->bytes/sizeof(float); i++) {
        kString += std::to_string(*ptr);
  ...
  char* path = nullptr;
  if (tflite::mtk::GetOpDumpPath(&path)) {
    std::ofstream kFile;

    kFile.open(path, std::ofstream::out | std::ofstream::app);
    if (kFile.is_open()) {
      for (int i = 0; i < kString.length(); i+= 800) {
          kFile << kString.substr(i, 800) << "\n";
      }
      kFile.close();
    }
  } else {
      for (int i = 0; i < kString.length(); i+= 800) {
        TFLITE_MTK_LOG_OP_DUMP("%s", kString.substr(i,800).c_str());
      }
  }
...
TfLiteStatus Subgraph::PrepareOpsAndTensors() {
  if (!memory_planner_) {
    /// M: NeuroPilot @{
    bool preserve_intermediates = tflite::mtk::IsDumpOpSupported();
    /// M: NeuroPilot @}
    memory_planner_.reset(new ArenaPlanner(
        &context_, std::unique_ptr<GraphInfo>(new InterpreterInfo(this)),
        /*preserve_inputs=*/true, preserve_intermediates,
...
TfLiteStatus Subgraph::Invoke() {
  ...
  if (status == kTfLiteOk && tflite::mtk::IsDumpOpSupported()) {
    for (size_t execution_plan_index = 0;
      ...
      for (int i = 0; i < node.outputs->size; i++) {
        tensor_list.push_back(tensors()[node.outputs->data[i]]);
      }
      DumpOpOutput((BuiltinOperator)registration.builtin_code, tensor_list);
    }
  }
  /// M: NeuroPilot @}
  return status;

...
class CustomOpHelper {
 public:
  ...
  typedef TfLiteStatus (*ParameterFuncNeuron)(void*, NeuronModel*,
                                        std::vector<uint32_t>&, uint32_t&);
  typedef int32_t (*MtkExtOpParameterFunc)(ANeuralNetworksModel*,
                                        std::vector<uint32_t>&, uint32_t&, void*);
  typedef int32_t (*MtkExtOpParameterFuncNeuron)(NeuronModel*,
                                        std::vector<uint32_t>&, uint32_t&, void*);
  ...
  void SetParamsFuncNeuron(const char* op_name, const char* target_name,
  ...
  ParameterFuncNeuron GetParamFuncNeuron(std::string op_name) {
    std::lock_guard<std::mutex> lock(index_addparams_mutex);
    std::unordered_map<std::string, ParameterFuncNeuron>::const_iterator got =
        index_addparams_neuron_.find(op_name);
    if (got != index_addparams_neuron_.end()) {
      return got->second;
    } else {
      return nullptr;
    }
  ...
  void SetMtkExtOpParameterFuncNeuron(std::string op_name, MtkExtOpParameterFuncNeuron add_params) {
    std::lock_guard<std::mutex> lock(mtk_ext_op_addparams_mutex);
    mtk_ext_op_addparams_neuron_.insert(make_pair(op_name, add_params));
  }

  MtkExtOpParameterFuncNeuron GetMtkExtOpParamFuncNeuron(std::string op_name) {
    std::lock_guard<std::mutex> lock(mtk_ext_op_addparams_mutex);
    std::unordered_map<std::string, MtkExtOpParameterFuncNeuron>::const_iterator got =
        mtk_ext_op_addparams_neuron_.find(op_name);
    if (got != mtk_ext_op_addparams_neuron_.end()) {
      return got->second;
    } else {
      return nullptr;
    }
 ...
 private:
  ...
  std::unordered_map<std::string, ParameterFuncNeuron> index_addparams_neuron_;
  std::unordered_map<std::string, MtkExtOpParameterFunc> mtk_ext_op_addparams_;
  std::unordered_map<std::string, MtkExtOpParameterFuncNeuron> mtk_ext_op_addparams_neuron_;

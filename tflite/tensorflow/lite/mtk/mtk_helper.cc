...
void CustomOpHelper::SetParamsFunc(const char* op_name,
  ...
  TFLITE_MTK_LOG_INFO("SetParamsFunc, OP: %s, Vendor: %s",
                      op_name, vendor_name);
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("SetParamsFunc, OP name should not be null");
      return;
  }
  std::string full_name = std::string(op_name) +
                          std::string(":") +
                          (target_name != nullptr ? std::string(target_name) : "") +
                          std::string(":") +
                          (vendor_name != nullptr ? std::string(vendor_name) : "");

  int32_t op_hash = mtk::Hash(full_name.c_str());
  TFLITE_MTK_LOG_INFO("Hash value in 32bit: %d", op_hash);
  {
...
void CustomOpHelper::SetParamsFuncNeuron(const char* op_name,
  const char* target_name,
  const char* vendor_name, ParameterFuncNeuron add_params) {
  TFLITE_MTK_LOG_INFO("SetParamsFuncNeuron, OP: %s, Vendor: %s",
                      op_name, vendor_name);
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("SetParamsFuncNeuron, OP name should not be null");
      return;
  }
  std::string full_name = std::string(op_name) +
                          std::string(":") +
  ...
  {
      std::lock_guard<std::mutex> lock(index_addparams_mutex);
      index_addparams_neuron_.insert(std::make_pair(op_name, add_params));
  }
  {
      std::lock_guard<std::mutex> lock(index_hash_mutex);
      index_hash_.insert(std::make_pair(op_name, op_hash));
...
TfLiteStatus CustomOpHelper::SetOutputDims(const char* op_name,
                                          ...
                                          int dims_size,
                                          int* dims) {
  TFLITE_MTK_LOG_INFO("SetOutputDims, OP: %s", op_name);
  ...
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("SetOutputDims, op_name should not be null");
      return kTfLiteError;
  }
...
TfLiteStatus CustomOpHelper::GetOutputRank(const char* op_name,
  ...
  std::lock_guard<std::mutex> lock(index_output_dims_mutex);
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("GetOutputRank, op_name should not be null");
      return kTfLiteError;
  }
  std::unordered_map<std::string,
                    std::vector<std::vector<int>>>::iterator got =
...
TfLiteStatus CustomOpHelper::GetOutputDims(const char* op_name,
  ...
  std::lock_guard<std::mutex> lock(index_output_dims_mutex);
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("GetOutputDims, op_name should not be null");
      return kTfLiteError;
  }
  std::unordered_map<std::string,
                    std::vector<std::vector<int>>>::iterator got =
...
TfLiteStatus CustomOpHelper::AddParam(const char* op_name,
  ...
  TFLITE_MTK_LOG_INFO("AddParam, OP: %s, type: %d", op_name, type);
  ...
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("AddParam, OP name should not be null");
      ret = kTfLiteError;
  }
  if (ret == kTfLiteOk) {
...
std::vector<CustomOpParam>* CustomOpHelper::GetParams(const char* op_name) {
  TFLITE_MTK_LOG_INFO("GetParams, OP: %s", op_name);
  std::lock_guard<std::mutex> lock(index_params_mutex);
  if (op_name == nullptr) {
      TFLITE_MTK_LOG_INFO("GetParams, OP name should not be nullptr");
      return nullptr;
  }
  std::unordered_map<std::string, std::vector<CustomOpParam>>::iterator got =
      index_params_.find(std::string(op_name));

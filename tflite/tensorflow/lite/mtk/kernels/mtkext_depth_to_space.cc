...
#define LOG_TAG "MtkExtDepthToSpace"
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_EQ(context, SizeOfDimension(input, 3) % (block_size * block_size), 0);
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  context->ReportError(context, "MTKEXT_DEPTH_TO_SPACE operator is not implemented.\n");
  return kTfLiteError;
}
...
int32_t add_neuron_params(NeuronModel* nn_model,
  ...
  auto builtin = reinterpret_cast<ops::mtkext::depth_to_space::OpData*>(data);
...
TfLiteRegistration* Register_MTKEXT_DEPTH_TO_SPACE() {
  static TfLiteRegistration r = {depth_to_space::Init,
                                 depth_to_space::Free,
                                 depth_to_space::Prepare,
                                 depth_to_space::Eval};
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFuncNeuron(
      "MTKEXT_DEPTH_TO_SPACE", depth_to_space::add_neuron_params);
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFunc(
      "MTKEXT_DEPTH_TO_SPACE", depth_to_space::add_ann_params);
  return &r;
}

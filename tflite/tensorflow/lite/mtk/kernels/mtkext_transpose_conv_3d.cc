...
#define LOG_TAG "MtkExtTransposeConv3d"
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_OK(contesxt, GetInputSafe(context, node, 3, &bias));
  ...
  TF_LITE_ENSURE_EQ(context, NumElements(output_shape), NumDimensions(input));

  if (output_shape->type != kTfLiteInt32) {
    context->ReportError(context, "Output shape is %d, not int32.", output_shape->type);
    return kTfLiteError;
  }
  ...
  auto ret = context->ResizeTensor(context, output, output_shape_array);
  if (ret == kTfLiteOk) {
    // Check if the input width/height are valid.
    ...
    auto output_width = SizeOfDimension(output, 3);
    auto output_height = SizeOfDimension(output, 2);
    auto output_depth = SizeOfDimension(output, 1);
    auto padding = data->padding;
    int expected_width = ComputeOutSize(padding, output_width, filter_width, data->stride_width,
                                        data->dilation_width_factor);
    int expected_height = ComputeOutSize(padding, output_height, filter_height, data->stride_height,
                                         data->dilation_height_factor);
    int expected_depth = ComputeOutSize(padding, output_depth, filter_depth, data->stride_depth,
                                         data->dilation_depth_factor);
    TF_LITE_ENSURE_EQ(context, width, expected_width);
    TF_LITE_ENSURE_EQ(context, height, expected_height);
    TF_LITE_ENSURE_EQ(context, depth, expected_depth);
  }
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  context->ReportError(context, "MTKEXT_TRANSPOSE_CONV_3D operator is not implemented.\n");
  return kTfLiteError;
}
...
int32_t add_neuron_params(NeuronModel* nn_model,
  ...
  auto builtin = reinterpret_cast<ops::mtkext::transpose_conv_3d::OpData*>(data);
...
TfLiteRegistration* Register_MTKEXT_TRANSPOSE_CONV_3D() {
  static TfLiteRegistration r = {transpose_conv_3d::Init,
                                 transpose_conv_3d::Free,
                                 transpose_conv_3d::Prepare,
                                 transpose_conv_3d::Eval};
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFuncNeuron(
      "MTKEXT_TRANSPOSE_CONV_3D", transpose_conv_3d::add_neuron_params);
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFunc(
      "MTKEXT_TRANSPOSE_CONV_3D", transpose_conv_3d::add_ann_params);

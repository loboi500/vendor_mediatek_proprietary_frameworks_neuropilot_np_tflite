...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (data_type == kTfLiteUInt8 || data_type == kTfLiteInt16 || data_type == kTfLiteInt8) {
...
TfLiteRegistration* Register_MTK_DEPTH_SPACE_TO_REF() {
  ...
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFuncNeuron(
      "MTK_DEPTH_TO_SPACE", depth_to_space::add_neuron_params);
  return &r;
}

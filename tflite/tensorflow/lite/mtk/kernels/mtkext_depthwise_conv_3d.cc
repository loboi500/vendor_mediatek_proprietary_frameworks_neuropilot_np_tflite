...
#define LOG_TAG "MtkExtDepthwiseConv3d"
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_EQ(context, SizeOfDimension(filter, 4), SizeOfDimension(bias, 0));
  TF_LITE_ENSURE_EQ(context, SizeOfDimension(input, 4) * data->depth_multiplier,
                    SizeOfDimension(filter, 4));

  int channels_out = SizeOfDimension(filter, 4);
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  context->ReportError(context, "MTKEXT_DEPTHWISE_CONV_3D operator is not implemented.\n");
  return kTfLiteError;
}
...
int32_t add_neuron_params(NeuronModel* nn_model,
  ...
  auto builtin = reinterpret_cast<ops::mtkext::depthwise_conv_3d::OpData*>(data);
...
TfLiteRegistration* Register_MTKEXT_DEPTHWISE_CONV_3D() {
  static TfLiteRegistration r = {depthwise_conv_3d::Init,
                                 depthwise_conv_3d::Free,
                                 depthwise_conv_3d::Prepare,
                                 depthwise_conv_3d::Eval};
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFuncNeuron(
      "MTKEXT_DEPTHWISE_CONV_3D", depthwise_conv_3d::add_neuron_params);
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFunc(
      "MTKEXT_DEPTHWISE_CONV_3D", depthwise_conv_3d::add_ann_params);

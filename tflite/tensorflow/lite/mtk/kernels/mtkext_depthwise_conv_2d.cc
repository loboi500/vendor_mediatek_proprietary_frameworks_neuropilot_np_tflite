...
#define LOG_TAG "MtkExtDepthWiseConv2d"
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_EQ(context, SizeOfDimension(filter, 3), SizeOfDimension(bias, 0));
  TF_LITE_ENSURE_EQ(context, SizeOfDimension(input, 3) * data->depth_multiplier,
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  context->ReportError(context, "MTKEXT_DEPTHWISE_CONV_2D operator is not implemented.\n");
  return kTfLiteError;
}
...
int32_t add_neuron_params(NeuronModel* nn_model,
  ...
  auto builtin = reinterpret_cast<ops::mtkext::depthwise_conv_2d::OpData*>(data);
...
TfLiteRegistration* Register_MTKEXT_DEPTHWISE_CONV_2D() {
  static TfLiteRegistration r = {depthwise_conv_2d::Init,
                                 depthwise_conv_2d::Free,
                                 depthwise_conv_2d::Prepare,
                                 depthwise_conv_2d::Eval};
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFuncNeuron(
      "MTKEXT_DEPTHWISE_CONV_2D", depthwise_conv_2d::add_neuron_params);
  ::tflite::mtk::CustomOpHelper::GetInstance().SetMtkExtOpParameterFunc(
      "MTKEXT_DEPTHWISE_CONV_2D", depthwise_conv_2d::add_ann_params);

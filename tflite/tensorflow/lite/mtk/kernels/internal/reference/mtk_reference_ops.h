...
void ConvertTensorDataToInt32(const int32_t size, const void* data,
                                      const TfLiteType type,
                                      std::vector<int32_t>* int32_data) {
  TF_LITE_ASSERT(type == TfLiteType::kTfLiteUInt8 ||
                 type == TfLiteType::kTfLiteInt8 ||
                 type == TfLiteType::kTfLiteInt16);
  int32_data->resize(size);
  if (type == TfLiteType::kTfLiteUInt8) {
    const uint8_t* ptr = reinterpret_cast<const uint8_t*>(data);
    for (int32_t idx = 0 ; idx < size ; idx ++) {
      int32_data->at(idx) = static_cast<int32_t>(*(ptr+idx));
    }
...
void ConvertInt32ToOutputData(const int32_t size, void* data,
                                      const TfLiteType type,
                                      const std::vector<int32_t>* int32_data) {
  TF_LITE_ASSERT(type == TfLiteType::kTfLiteUInt8 ||
                 type == TfLiteType::kTfLiteInt8 ||
                 type == TfLiteType::kTfLiteInt16);
  if (type == TfLiteType::kTfLiteUInt8) {
    ...
    for (int32_t idx = 0 ; idx < size ; idx ++) {
      ptr[idx] = static_cast<uint8_t>((*int32_data)[idx]);
    }
  } else if (type == TfLiteType::kTfLiteInt8) {
    int8_t* ptr = reinterpret_cast<int8_t*>(data);
    for (int32_t idx = 0 ; idx < size ; idx ++) {
      ptr[idx] = static_cast<int8_t>((*int32_data)[idx]);
    }
  } else if (type == TfLiteType::kTfLiteInt16) {
    int16_t* ptr = reinterpret_cast<int16_t*>(data);
    for (int32_t idx = 0 ; idx < size ; idx ++) {
      ptr[idx] = static_cast<int16_t>((*int32_data)[idx]);
    }
  }
}
...
inline void MinPool(const MtkPoolParams& params, const RuntimeShape& input_shape,
...
inline void MinPoolRequantize(const MtkPoolParams& params,
                              const RuntimeShape& input_shape,
                              const void* input_data, const TfLiteType itype,
                              const RuntimeShape& output_shape,
                              void* output_data, const TfLiteType otype) {
  std::vector<int32_t> int32_input_data;
  std::vector<int32_t> int32_output_data;
  int32_output_data.resize(output_shape.FlatSize());

  ConvertTensorDataToInt32(input_shape.FlatSize(), input_data, itype, &int32_input_data);
        ...
        for (int channel = 0; channel < depth; ++channel) {
          ...
          for (int filter_y = filter_y_start; filter_y < filter_y_end;
            ...
            for (int filter_x = filter_x_start; filter_x < filter_x_end;
              ...
              const int in_x = in_x_origin + filter_x;
              const int in_y = in_y_origin + filter_y;
              int32 input_val = int32_input_data[Offset(input_shape, batch,
                                                        in_y, in_x, channel)];
  ...
  ConvertInt32ToOutputData(output_shape.FlatSize(), output_data, otype, &int32_output_data);
}
...
inline void LayerNormalization(const float* input_data, const int* input_dims,
  ...
  reference_ops::BroadcastSubSlow(params, tf_input_dims, input_data,
                                    tf_temp_dims, temp_mean,
                                    tf_output_dims, output_data);
  reference_ops::BroadcastDivSlow(params, tf_output_dims, output_data,

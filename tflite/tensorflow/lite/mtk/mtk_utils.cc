...
static void InitOptions(const char* key) {
  ...
  if (length == 0) {
    TFLITE_MTK_LOG_INFO("Empty NeuroPilot feature property");
    return;
  }
  ...
  for (const auto& elem : elements) {
    ...
    if (index < 0 || index > 25) {
      TFLITE_MTK_LOG_ERROR("Invalid index: %d", index);
      continue;
    }

    TFLITE_MTK_LOG_INFO("Property index: %d", index);
    sOptions[index] = true;
  }
#endif  // __ANDROID__
}
...
static bool OverwriteOptions() {
  ...
  if (__system_property_get("debug.nn.mtk_nn.option", var) != 0) {
    TFLITE_MTK_LOG_INFO(
        "For debug: feature support by debug proerty: %s", var);
...
static int32_t GetAndroidSdkVersion() {
  ...
  TFLITE_MTK_LOG_ERROR("No %s prop", sdkProp);
  return 0;
#endif  // __ANDROID__
  return 0;
}
...
int32_t Hash(const char* str) {
  ...
  TFLITE_MTK_LOG_INFO("Hash string: %s", str);
...
bool IsFeatureSupported(int feature) {
  if (feature < 0 || feature > 25) {
    TFLITE_MTK_LOG_ERROR("Unknown feature: %d", feature);
    return false;
  }
...
int64_t GetMtkNnApiDelegateFlag(void) {
  static bool enabled = tflite::mtk::PropertyGetBool(
      "debug.mtk_tflite.allow_dynamic_tensor", false);
  return (enabled ? kTfLiteDelegateFlagsAllowDynamicTensors
                  : kTfLiteDelegateFlagsNone);
}

bool IsMtkCustomNode(void* node) {
  TfLiteNode* node_to_check = reinterpret_cast<TfLiteNode*>(node);
  if (node_to_check->custom_initial_data == nullptr) {
    TFLITE_MTK_LOG_ERROR("unsupported custom node, null custom_initial_data");
    return false;
}
  auto map =
      flexbuffers::GetRoot((const uint8_t*)node_to_check->custom_initial_data,
                            node_to_check->custom_initial_data_size)
          .AsMap();
  auto op_type = map["op_type"].AsString();
  auto target_device = map["target_device"].AsString();
  auto vendor = map["vendor"].AsString();
  if (op_type.length() == 0 || target_device.length() == 0 ||
      vendor.length() == 0) {
    TFLITE_MTK_LOG_ERROR("unsupported custom node");
    return false;
  }
  TFLITE_MTK_LOG_INFO("supported custom node");
  return true;
}

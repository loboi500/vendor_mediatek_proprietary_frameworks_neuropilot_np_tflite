...
class COUT_PRINT_ERROR {
public:
  std::stringstream input_str;

  ~COUT_PRINT_ERROR() { TFLITE_MTK_LOG_ERROR("%s", input_str.str().c_str()); }

  template <typename T> COUT_PRINT_ERROR &operator<<(T a) {
    input_str << a;
    return *this;
  }
};
...
public:
  ...
  NeuronDelegateOptions options_neuron_;
...
TfLiteStatus TFLiteRunner::Run(const std::vector<std::string> &xs_list,
  ...
  for (int32_t i = 0; i < loop_count_; i++) {
    for (size_t j = 0; j < xs_list.size(); j++) {
      ...
      if (i == 0 && j == 0) {
        LOG(TFLITE_INFO) << "Model preparation + OP support query + Inference : "
                  << elapsed.count() * 1000 << " ms";
      } else {
        LOG(TFLITE_INFO) << "Inference time : " << elapsed.count() * 1000 << " ms";
      }
...
TfLiteStatus TFLiteRunner::RunWithRandomInput(void) {
  ...
  if (loop_count_ > 1) {
    ...
    LOG(TFLITE_INFO) << "Avg Inference time: " << avg_inference_time << " ms";
  } else {
...
TfLiteStatus TFLiteRunner::PrepareInput(int tensor_id, cnpy::NpyArray array) {
  return PrepareInput(tensor_id,
                      reinterpret_cast<void *>(array.data<uint8_t>()),
                      array.num_bytes());
}
...
TfLiteStatus TFLiteRunner::CompareOutput(int tensor_id, const char *batch_ys) {
  ...
  case kTfLiteFloat32:
    result =
        CompareOutput<float>(tensor_id, reinterpret_cast<float *>(loaded_data));
    break;
  case kTfLiteInt32:
...
TfLiteStatus TFLiteRunner::CompareOutput(int tensor_id, T *golden_data) {
  ...
  LOG(TFLITE_INFO) << "Result is matched!!";
  return kTfLiteOk;
}
...
int TFLiteRunner_Create(TFLiteRunnerHandle **handle,
  ...
  resolver.AddOtherCustom(model->GetModel());
...
void TFLiteRunner_UseNnApiDelegate(TFLiteRunnerHandle *handle) {
  ...
  r->delegate_ = new tflite::StatefulNnApiDelegate(r->options_);
  r->GetInterpreter()->ModifyGraphWithDelegate(r->delegate_);
}
...
void TFLiteRunner_UseNeuronDelegate(TFLiteRunnerHandle *handle) {
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->delegate_ = TfLiteNeuronDelegateCreate(&r->options_neuron_);
  r->GetInterpreter()->ModifyGraphWithDelegate(r->delegate_);
}
...
void TFLiteRunner_SetLowLatency(TFLiteRunnerHandle *handle, bool enableLowLatency) {
  ...
  CHECK_NOTNULL(r->GetInterpreter());
  if (enableLowLatency) {
      r->options_neuron_.optimization_hint |=
        OptimizationHint::kOptimizationLowLatency;
  } else {
      r->options_neuron_.optimization_hint &=
        ~OptimizationHint::kOptimizationLowLatency;
  }
...
void TFLiteRunner_SetDeepFusion(TFLiteRunnerHandle *handle, bool enableDeepFusion) {
  ...
  if (enableDeepFusion) {
      r->options_neuron_.optimization_hint |=
        OptimizationHint::kOptimizationDeepFusion;
  } else {
      r->options_neuron_.optimization_hint &=
        ~OptimizationHint::kOptimizationDeepFusion;
  }
...
void TFLiteRunner_SetBatchProcessing(TFLiteRunnerHandle *handle, bool enableBatchProcessing) {
  ...
  if (enableBatchProcessing) {
      r->options_neuron_.optimization_hint |=
        OptimizationHint::kOptimizationBatchProcessor;
  } else {
      r->options_neuron_.optimization_hint &=
        ~OptimizationHint::kOptimizationBatchProcessor;
  }
...
void TFLiteRunner_SetCacheDir(TFLiteRunnerHandle *handle,
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.cache_dir = cache_dir;
  r->options_neuron_.cache_dir = cache_dir;
}
...
void TFLiteRunner_SetPreference(TFLiteRunnerHandle *handle,
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.execution_preference =
    (tflite::StatefulNnApiDelegate::Options::ExecutionPreference)execution_preference;
  r->options_neuron_.execution_preference =
...
void TFLiteRunner_SetDisallowNnApiCpu(TFLiteRunnerHandle* handle, bool disallow_nnapi_cpu) {
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.disallow_nnapi_cpu = disallow_nnapi_cpu;
}
...
void TFLiteRunner_SetNoSupportedOperationCheck(TFLiteRunnerHandle* handle, bool no_supported_operation_check) {
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_neuron_.no_supported_operation_check = no_supported_operation_check;
}
...
void TFLiteRunner_SetAcceleratorName(TFLiteRunnerHandle* handle, const char* accelerator_name) {
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.accelerator_name = accelerator_name;
  r->options_neuron_.accelerator_name = accelerator_name;
}
...
void TFLiteRunner_SetExecutionPriority(TFLiteRunnerHandle* handle,  int execution_priority) {
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.execution_priority = execution_priority;
  r->options_neuron_.execution_priority =
...
void TFLiteRunner_SetMaxCompilationTimeout(TFLiteRunnerHandle* handle,
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.max_compilation_timeout_duration_ns = max_compilation_timeout_duration_ns;
}
...
void TFLiteRunner_SetMaxExecutionTimeout(TFLiteRunnerHandle* handle,
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.max_execution_timeout_duration_ns = max_execution_timeout_duration_ns;
}
...
void TFLiteRunner_SetMaxExecutionLoopTimeout(TFLiteRunnerHandle* handle,
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_.max_execution_loop_timeout_duration_ns = max_execution_loop_timeout_duration_ns;
}
...
void TFLiteRunner_SetAllowFp16Precision(TFLiteRunnerHandle *handle,
  ...
  r->options_neuron_.allow_fp16 = allow;
}
...
void TFLiteRunner_SetBoostDuration(TFLiteRunnerHandle* handle,
  ...
  CHECK_NOTNULL(r);
  CHECK_NOTNULL(r->GetInterpreter());
  r->options_neuron_.boost_duration = duration;
}

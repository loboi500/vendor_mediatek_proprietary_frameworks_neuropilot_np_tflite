...
#include <iostream>
#include <array>
#define NNAPI_VERBOSE_VALIDATION
/// M: NeuroPilot @}
...
/// M: NeuroPilot {@
enum {
    ANN_EXTENSION_GENERAL_OPERAND_ARGSTRING = 0x0100,
    ANN_EXTENSION_GENERAL_OPERATION_TYPE = 0x0000,
};
static constexpr const char* kAnnExtensionGeneralOpration = "com.mediatek.general_operation";
/// M: NeuroPilot @}
...
#define RETURN_TFLITE_ERROR_IF_ION_ERROR(code)                \
  do {                                                        \
    const auto _code = (code);                                \
    if (_code != 0) {                                         \
      TFLITE_MTK_LOG_ERROR("Ion lock/unlock fail %d", _code); \
      return kTfLiteError;                                    \
    }                                                         \
  } while (0)
...
constexpr size_t kDefaultByteAlignmentForNNAPI = 128;
...
TfLiteStatus GetTargetDevices(TfLiteContext* context, TfLiteDelegate* delegate,
  ...
  TFLITE_MTK_LOG_INFO("disallow_nnapi_cpu %d", delegate_options.disallow_nnapi_cpu);
  if (device_name_ptr != nullptr) {
    // User specified an accelerator to use.
    ANeuralNetworksDevice* nnapi_device = nullptr;
    TFLITE_MTK_LOG_INFO("device_name_ptr %s", device_name_ptr);
  ...
  if (name && size > 0) {
    ...
    if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI12) {
      ...
      if (AHardwareBuffer_allocate(&desc, &buffer_) == 0) {
        nnapi_->ANeuralNetworksMemory_createFromAHardwareBuffer(
...
class NNAPIOpBuilder {
 public:
  ...
  /// M: NeuroPilot {@
  TfLiteStatus AddScalarOemOperand(const uint8_t* values, uint32_t num_values) {
  ...
  void SwapInput(int first_index, int second_index) {
    std::iter_swap(augmented_inputs_.begin() + first_index,
                   augmented_inputs_.begin() + second_index);
  }
  /// M: NeuroPilot @}
 private:
  ...
  TfLiteStatus AddVectorOperand(const T* values, uint32_t num_values,
    ...
    ANeuralNetworksOperandType operand_type{.type = nn_type,
                                            ...
                                            .zeroPoint = zero_point};
    /// M: NeuroPilot {@
    if (nn_type == ANEURALNETWORKS_OEM_SCALAR) {
      if(nnapi_->android_sdk_version > kMinSdkVersionForNNAPI13) {
          int32_t extension_type;
          ANeuralNetworksModel_getExtensionOperandType(nn_model_, kAnnExtensionGeneralOpration,
                  ANN_EXTENSION_GENERAL_OPERAND_ARGSTRING, &extension_type);
          operand_type.type = extension_type;
      } else {
  ...
  TfLiteStatus AddTensor(int tensor_index, bool hybrid_op,
    ...
    switch (tensor_type) {
      ...
      case kTfLiteInt16:
        ...
        zeroPoint = tensor->params.zero_point;
        if (scale == 0) {
            // QUANT tensors with zero scale are not valid in NNAPI.
            scale = 1;
        }
        break;
      default:
...
bool NNAPIDelegateKernel::Validate(
    const TfLiteContext* context, TfLiteRegistration *registration, int builtin_code, int version,
  ...
  switch (builtin_code) {
    ...
    case kTfLiteBuiltinSoftmax: {
      Expect(GetPropertyValue("ro.soc.model") != "mt5896",
             NNAPIValidationFailureType::kUnsupportedOperator,
             "softmax don't support in this platform", &val_ctx);
    ...
    /// M: NeuroPilot {@
    case kTfLiteBuiltinCustom: {
      Expect(registration->custom_name != nullptr,
              NNAPIValidationFailureType::kUnsupportedOperator,
              "Empty custom name form a kTfLiteBuiltinCustom OP", &val_ctx);

      if (strcmp(registration->custom_name, "MTK_OPT") == 0) {
      } else {
        // Process MTK builtin OP here for NNAPI 1.1 HAL
        if (strcmp(registration->custom_name, "MTK_DEPTH_TO_SPACE") == 0) {
        } else if (strncmp(registration->custom_name, "MTK", 3) == 0) {
        } else if (strncmp(registration->custom_name, "mcv_", 4) == 0 ||
                strncmp(registration->custom_name, "cust_", 4) == 0 ||
                strncmp(registration->custom_name, "halidegen_", 4) == 0) {
        } else {
          Expect(tflite::mtk::CustomOpHelper::GetInstance().GetParamFunc(registration->custom_name)
                  != nullptr,
                  NNAPIValidationFailureType::kUnsupportedOperator,
                  "Empty custom param func form a kTfLiteBuiltinCustom OP", &val_ctx);
        }
      }
    } break;
    /// M: NeuroPilot @}
...
TfLiteStatus NNAPIDelegateKernel::Map(
  ...
  switch (builtin_code) {
    ...
    case kTfLiteBuiltinCustom: {
      ...
      if (strcmp(mapping_args.registration->custom_name, "MTK_OPT") == 0) {
          *nn_op_type = ANEURALNETWORKS_OEM_OPERATION;
          if (android_sdk_version > kMinSdkVersionForNNAPI13) {
              ANeuralNetworksModel_getExtensionOperationType(mapping_args.builder->GetNnModel(),
                      kAnnExtensionGeneralOpration, ANN_EXTENSION_GENERAL_OPERATION_TYPE,
                                                             nn_op_type);
          }
      } else {
        // Process MTK builtin OP here for NNAPI 1.1 HAL
        if (strcmp(mapping_args.registration->custom_name, "MTK_DEPTH_TO_SPACE") == 0 ||
                strcmp(mapping_args.registration->custom_name, "MTKEXT_DEPTH_TO_SPACE") == 0) {
            uint32_t next_id =
              mapping_args.builder->GetNextAnnOperandIdx();
            ::tflite::mtk::CustomOpHelper::MtkExtOpParameterFunc mtk_op_param_func =
              ::tflite::mtk::CustomOpHelper::GetInstance().GetMtkExtOpParamFunc(
            ...
            *nn_op_type = ANEURALNETWORKS_DEPTH_TO_SPACE;
        } else if (strncmp(mapping_args.registration->custom_name, "MTK", 3) == 0) {
            ...
            *nn_op_type = ANEURALNETWORKS_OEM_OPERATION;
        ...
        } else if (strncmp(mapping_args.registration->custom_name, "mcv_", 4) == 0 ||
                strncmp(mapping_args.registration->custom_name, "cust_", 4) == 0 ||
                strncmp(mapping_args.registration->custom_name, "halidegen_", 4) == 0) {
            ...
            size_t oem_scalar_size = 0;
            uint8_t* oem_scalar = nullptr;
            std::string s(mapping_args.registration->custom_name);
            s += ":common:mediatek";
          ...
          // Process 3rd party custom OP for NNAPI 1.1 HAL
            uint32_t next_id =
              mapping_args.builder->GetNextAnnOperandIdx();
            tflite::mtk::CustomOpHelper::ParameterFunc custom_op_param_func =
              ::tflite::mtk::CustomOpHelper::GetInstance().GetParamFunc(
                mapping_args.registration->custom_name);
            if (custom_op_param_func != nullptr) {
            ...
            const char* target =
                tflite::mtk::CustomOpHelper::GetInstance().
                  FindTarget(mapping_args.registration->custom_name);
            if (target != nullptr) {
...
TfLiteStatus NNAPIDelegateKernel::Init(TfLiteContext* context,
  ...
  if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI12 && cache_dir) {
    ...
    model_token = bytes_count_str.c_str();
...
TfLiteStatus NNAPIDelegateKernel::Prepare(TfLiteContext* context,
  ...
  if (preference !=
    ...
    TFLITE_MTK_LOG_INFO("ANeuralNetworksCompilation_setPreference %d", preference);
  ...
  if (!nn_compilation_cache_token_.empty()) {
    const char* cache_dir = delegate_options.cache_dir;
    TFLITE_MTK_LOG_INFO("ANeuralNetworksCompilation_setCaching %s", cache_dir);
  ...
  if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI13) {
    if (delegate_options.max_compilation_timeout_duration_ns > 0) {
      TFLITE_MTK_LOG_INFO("ANeuralNetworksCompilation_setTimeout %d",
    ...
    TFLITE_MTK_LOG_INFO("ANeuralNetworksCompilation_setPriority %d",
            delegate_options.execution_priority);
    RETURN_TFLITE_ERROR_IF_NN_ERROR(
        context,
...
TfLiteStatus NNAPIDelegateKernel::Invoke(TfLiteContext* context,
  ...
  if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI13) {
    if (delegate_options.max_execution_timeout_duration_ns > 0) {
      TFLITE_MTK_LOG_INFO("ANeuralNetworksExecution_setTimeout %d",
    ...
    if (delegate_options.max_execution_loop_timeout_duration_ns > 0) {
      TFLITE_MTK_LOG_INFO("ANeuralNetworksExecution_setLoopTimeout %d",
...
TfLiteStatus NNAPIDelegateKernel::AddOpsAndTensors(
  ...
  for (auto node_index : nodes_) {
    ...
    /// M: NeuroPilot {@
    TFLITE_MTK_LOG_INFO("Process builtin OP(%d):%s", reg->builtin_code,
      EnumNameBuiltinOperator(
        static_cast<BuiltinOperator>(reg->builtin_code)));
    TFLITE_MTK_LOG_INFO("Num of input:%d", node->inputs->size);
    TFLITE_MTK_LOG_INFO("Num of output:%d", node->outputs->size);
    ...
    TF_LITE_ENSURE_STATUS(
        Map(context, reg->builtin_code, reg->version, target_sdk_version_,
            {context, &builder, node, node_index, &model_state_outputs_,
             &model_state_tfl_inputs_, &feedback_loops_, nnapi_errno, reg},
            &nn_op_type));
...
TfLiteStatus NNAPIDelegateKernel::BuildGraph(
  ...
  if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI11) {
    TFLITE_MTK_LOG_INFO("ANeuralNetworksModel_relaxComputationFloat32toFloat16 %d", allow_fp16);
    RETURN_TFLITE_ERROR_IF_NN_ERROR(
        context,
  ...
  *nnapi_errno = 0;
  ...
  if (nnapi->android_sdk_version >= kMinSdkVersionForNNAPI12) {
    if (ShouldUseTargetDevices(delegate_options, nnapi)) {
      ...
      // If device_count > 1 (there're other devices except for nnapi_reference),
      // set accelerator_flag = true to avoid is_accelerator_specified == false.
      accelerator_flag = true;
    }
  }
  ...
  for (int node_index : TfLiteIntArrayView(plan)) {
    ...
    std::vector<delegate::nnapi::NNAPIValidationFailure> failure;
    if (NNAPIDelegateKernel::Validate(context, registration, registration->builtin_code,
                                      registration->version, target_sdk_version,
                                      node, is_accelerator_specified || accelerator_flag, &failure)) {
      supported_nodes.push_back(node_index);
    } else {
      /// M: NeuroPilot {@
      TFLITE_LOG_PROD(tflite::TFLITE_LOG_ERROR, "OP (%d):%s is not supported(%s)",
                        registration->builtin_code,
                        EnumNameBuiltinOperator(static_cast<BuiltinOperator>(registration->builtin_code)),
                        failure[0].message.c_str());
      /// M: NeuroPilot @}
    }
  }
...
//M: NeuroPilot: remove getSupportedOperations by target to enhance performance
  ...
  TFLITE_MTK_LOG_INFO("max_number_delegated_partitions %d",
